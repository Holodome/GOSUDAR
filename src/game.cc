...
void *alloc(MemoryArena *arena, size_t size) {
    ...
    if (size) {
        if (!arena->minimum_block_size) {
            arena->minimum_block_size = MEGABYTES(1);
        }
        
        while (!result) {
            bool should_allocate_new_block = arena->current_block == 0;
            if (!should_allocate_new_block) {
                OSMemoryBlock *block = arena->current_block;
                uintptr_t curr_ptr = (uintptr_t)block->base + block->used;
                uintptr_t offset = align_forward(curr_ptr, 16) - (uintptr_t)block->base;
                if (offset + size < block->size) {
                    block->used = offset + size;
                    result = block->base + offset;
                } else {
            ...
            if (should_allocate_new_block) {
                OSMemoryBlock *new_block = os_alloc_block(arena->minimum_block_size);
