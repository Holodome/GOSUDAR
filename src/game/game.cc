...
void Game::init() {
    ...
    assets.arena.init(Mem::alloc(assets_arena_size), assets_arena_size);
    // @CLEAN
    this->assets.renderer = &this->renderer;
    this->assets.init("assets.cfg");
    this->renderer.white_texture = this->assets.get_tex(Asset_White);
    ...
    this->dev_ui.arena.init(Mem::alloc(devui_arena_size), devui_arena_size);
    this->dev_ui.init();
    this->dev_ui.assets = &this->assets;
...
void Game::cleanup() {
    ...
    Mem::free(this->dev_ui.arena.data);
    this->assets.cleanup();
...
void Game::update() {
    ...
    for (size_t y = 0; y < this->world.map_size.y; ++y) {
        for (size_t x = 0; x < this->world.map_size.x; ++x) {
            ...
            renderer.imm_draw_quad(&world_render_group, tilev, assets.get_tex(Asset_Grass));
        }
    }
    ...
    for (size_t drawable_idx = 0; drawable_idx < drawable_entity_id_count; ++drawable_idx) {
        ...
        renderer.imm_draw_quad(&world_render_group, billboard, assets.get_tex(entity->texture_id));
