...
static void 
out_stream_write_direct(OutStream *stream, const void *data, uptr data_sz) {
...
void 
init_out_stream(OutStream *stream, void *bf, uptr bf_sz) {
...
void 
init_out_streamf(OutStream *stream, OSFileHandle *file,  void *bf, uptr bf_sz, uptr threshold) {
...
uptr 
out_streamf(OutStream *stream, const char *format, ...) {
    va_list args;
...
uptr 
out_streamv(OutStream *stream, const char *format, va_list args) {
...
void 
out_streamb(OutStream *stream, const void *b, uptr c) {
...
void 
out_stream_flush(OutStream *stream) {
...
void 
init_in_stream(InStream *stream, void *bf, uptr bf_sz) {
    stream->mode = STREAM_BUFFER;
    stream->bf = (u8 *)bf;
    stream->bf_sz = bf_sz;
}
...
void 
init_in_streamf(InStream *stream, OSFileHandle *file, void *bf, uptr bf_sz, uptr threshold) {
...
uptr 
in_stream_peek(InStream *stream, void *out, uptr n) {
    uptr result = 0;
...
u8 
in_stream_soft_peek_at(InStream *stream, uptr offset) {
    u8 result = 0;
...
uptr 
in_stream_advance(InStream *stream, uptr n) {
    uptr result = 0;
    if (!stream->is_finished) {
        if (stream->bf_idx + n > stream->bf_used) {
            in_stream_flush(stream);
...
void 
in_stream_flush(InStream *stream) {
    if (stream->mode == STREAM_BUFFER) {
        // nop
    } else if (stream->mode == STREAM_FILE) {
...
u8 
in_stream_peek_b_or_zero(InStream *stream) {
    u8 result = 0;
